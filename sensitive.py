SECRET_KEY=''
DATABASE_HOST=''
DATABASE_NAME=''
DATABASE_USER=''
DATABASE_PASSWORD=''

# Flags for testing against a development MySQL database. This is used by default with AppEngine's dev_appserver,
# and can also be turned on by running 'export CAP_TOOLS_DB=dev', but this is not required if using Django's internal sqlite db.
DEV_DATABASE_HOST=''
DEV_DATABASE_NAME=''
DEV_DATABASE_USER=''
DEV_DATABASE_PASSWORD=''

# Set this variables to access production CloudSQL instance from development environment.
PROD_FROM_DEV_DATABASE_HOST=''
PROD_FROM_DEV_DATABASE_NAME=''
PROD_FROM_DEV_DATABASE_USER=''
PROD_FROM_DEV_DATABASE_PASSWORD=''
